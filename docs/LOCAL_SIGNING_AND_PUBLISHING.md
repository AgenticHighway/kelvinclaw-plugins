...
managed from the private `ah-infra` repo.
...
`arn:aws:iam::997752499028:role/ah-gha-kelvin-plugins-publisher` on a Blacksmith
...
1. `ah-infra` has already created the KMS signing key alias:
   - `alias/ah/kelvin/plugins/prod`
2. Your shell is using the expected AWS profile:

```bash
export AWS_PROFILE=ah-willsarg-iam
```
